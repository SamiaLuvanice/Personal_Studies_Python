...
def home(request):
    ...
    books = Book.objects.all() if request.user.is_authenticated else None
...
def register_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 
                'Você fez login com sucesso com novo usuário!'
            )
            return redirect('home')
...
def book_add(request):
    ...
    if request.user.is_authenticated:
        ...
        messages.error(
            request,
            'Você precisa estar logado para adicionar um livro!'
        )
        return redirect('home')
